...
imgs, name = load_images('_resources/_videos_must/video_stabilization_2.mp4', OUT_PATH='./_resources/_outputs/video_stabilization_result_drone/'), 'video_stabilization_result_drone'
...
plt.legend()
plt.xlabel('Frame')
plt.savefig('./_resources/_outputs/'+name+'_trajectory.png')
...
plt.savefig('./_resources/_outputs/'+name+'_smoothed.png')

#create a images that show both the trajectory and video frames
filenames = imshow_with_trajectory(images=new_imgs, warp_stack=ws-smoothed_warp, PATH='./_resources/_outputs/out_'+name+'/', ij=(0,2))

#create gif
create_gif(filenames, './_resources/_outputs/'+name+'.gif')
