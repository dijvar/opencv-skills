...
img = cv2.imread('_resources/_images_must/cats.jpg')
...
cv2.imwrite("_resources/_outputs/cropped_image.jpg", cropped_image)
 
cv2.waitKey(0)
cv2.destroyAllWindows()

# görüntüyü yamalara/parçalara ayırma
img =  cv2.imread("_resources/_outputs/cropped_image.jpg")
image_copy = img.copy()
...
for y in range(0, img_height, M):
    for x in range(0, img_width, N):
        ...
        if x1 >= img_width and y1 >= img_height:
            ...
            cv2.imwrite('_resources/_outputs/'+'tile'+str(x)+'_'+str(y)+'.jpg', tiles)
...
cv2.imwrite("_resources/_outputs/patched.jpg",img)
